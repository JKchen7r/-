...
file_path = r'D:\中医问答\pythonProject1\A1A2_data_txt\chap3_A1A2.txt'
...
j = 0
for item in result_list:
    regex = r"[a-eA-E]+[\．.]|【答案】|【解析】"
    result = re.split(regex, item)

    if len(result) != 8:
        result.append('')
    result.append(j)
    result.append('第三章：中药学')
    result.append('A1/A2')
...
with open('../A1A2_data_json/chap3_A1A2.json', 'w', encoding='utf-8') as file:
    file.write('[')
    file.write('\n')
